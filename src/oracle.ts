...
async function createAppConnectionPool(user: string, password: string, connectString: string) {
  // Close any existing pool in case credentials have changed
  try {
    await OracleDB.getPool().close(0);
  } catch (err) {
    if (err.code !== "NJS-047") {
      // Ignore "pool doesn't exist" error
      console.error(err);
    }
  }
...
async function doDateQuery() {
  ...
  try {
    ...
    const result = await connection.execute(
      `select to_char(current_timestamp, 'YYYY-MM-DD"T"hh:mi:ss.ff3"Z"') from dual`
    );
    ...
    if (connection) await connection.close(); // release the connection to the pool
  }
}

export { createAppConnectionPool, doDateQuery };
